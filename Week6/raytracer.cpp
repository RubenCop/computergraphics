...
Object* Raytracer::parseObject(const YAML::Node& node)
{
    ...
    if (objectType == "model"){
        ...
        Material *m = parseMaterial(node["material"]);;
        ...
        for (unsigned int i = 0; i < model->numtriangles; i++)
        {
            ...
            Triple a = Triple (model->vertices[3 * model->triangles[i].vindices[0]+0] + pos.x,
                model->vertices[3 * model->triangles[i].vindices[0]+1] + pos.y,
                model->vertices[3 * model->triangles[i].vindices[0]+2] + pos.z);

            Triple b = Triple (model->vertices[3 * model->triangles[i].vindices[1]+0] + pos.x,
